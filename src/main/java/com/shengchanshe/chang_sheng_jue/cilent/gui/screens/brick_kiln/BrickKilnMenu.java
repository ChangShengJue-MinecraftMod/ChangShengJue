...
public class BrickKilnMenu extends AbstractContainerMenu {
    ...
    public BrickKilnMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, MenuBlockEntityResolver.resolve(
                inv.player.level(), extraData.readBlockPos(), BrickKilnEntity.class,
                ChangShengJueBlocks.BRICK_KILN.get(), BrickKilnEntity::new), new SimpleContainerData(3));
    }

    public BrickKilnMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        this(pContainerId, inv, MenuBlockEntityResolver.resolve(
                inv.player.level(), entity, BrickKilnEntity.class,
                ChangShengJueBlocks.BRICK_KILN.get(), BrickKilnEntity::new), data);
    }

    private BrickKilnMenu(int pContainerId, Inventory inv,
                          MenuBlockEntityResolver.Resolution<BrickKilnEntity> resolution,
                          ContainerData data) {
        super(ChangShengJueMenuTypes.BRICK_KILN_MENU.get(), pContainerId);
    ...
    public boolean craftItem(Player player) {
        ...
        // 检查输出槽是否有物品
        if (!blockEntity.getItemHandler().getStackInSlot(BrickKilnEntity.SLOT_OUTPUT).isEmpty()) {
            return false;
        }
