...
public class InnkeeperMenu extends AbstractContainerMenu {
    ...
    private static final int ROW_Y = 78;
    ...
    public InnkeeperMenu(int id, Inventory inventory) {
        this(id, inventory, new ClientSideMerchant(inventory.player));
    }

    public InnkeeperMenu(int id, Inventory inventory, Merchant merchant) {
        super(ChangShengJueMenuTypes.INNKEEPER_MENU.get(), id);
        ...
        for(int i = 0; i < 3; ++i) {
            for(int j = 0; j < 9; ++j) {
                this.addSlot(new Slot(inventory, j + i * 9 + 9, 108 + j * 18, 125 + i * 18));
            }
        }

        for(int k = 0; k < 9; ++k) {
            this.addSlot(new Slot(inventory, k, 108 + k * 18, 183));
        }
