...
public class TheClassicsOfTendonChangingBook extends Item {
    public TheClassicsOfTendonChangingBook() {
        super(new Properties());
    }
    @Override
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if (!pLevel.isClientSide){
            pPlayer.getCapability(ChangShengJueCapabiliy.KUNGFU).ifPresent(cap -> cap.learnKungFu((ServerPlayer) pPlayer, TheClassicsOfTendonChanging.KUNG_FU_ID.toString()));
        }
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        if (TooltipInput.isShiftDown()) {
            Component fullDesc = Component.translatable("tooltip." + ChangShengJue.MOD_ID + "."
                    + this + ".hold_shift.tooltip", Component.translatable(KungFuConfig.THE_CLASSICS_OF_TENDON_CHANGING_MAX_EXP.get().toString())).withStyle(ChatFormatting.YELLOW);
