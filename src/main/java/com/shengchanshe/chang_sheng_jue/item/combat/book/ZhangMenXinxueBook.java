...
public class ZhangMenXinxueBook extends Item {
    public ZhangMenXinxueBook() {
        super(new Properties());
    }
    @Override
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if (!pLevel.isClientSide){
            pPlayer.getCapability(ChangShengJueCapabiliy.KUNGFU).ifPresent(cap -> cap.learnKungFu((ServerPlayer) pPlayer, ZhangMenXinXue.KUNG_FU_ID.toString()));
        }
