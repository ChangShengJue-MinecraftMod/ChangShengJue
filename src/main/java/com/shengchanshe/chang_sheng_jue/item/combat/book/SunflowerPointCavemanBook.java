...
public class SunflowerPointCavemanBook extends Item {
    public SunflowerPointCavemanBook() {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if (!pLevel.isClientSide){
            pPlayer.getCapability(ChangShengJueCapabiliy.KUNGFU).ifPresent(cap -> {
                cap.learnKungFu((ServerPlayer) pPlayer, SunflowerPointCaveman.KUNG_FU_ID.toString());
                cap.startKungFu((ServerPlayer) pPlayer, SunflowerPointCaveman.KUNG_FU_ID.toString(), SunflowerPointCaveman.KUNG_FU_ID.getPath());
            });
        }
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        if (TooltipInput.isShiftDown()) {
            Component fullDesc = Component.translatable("tooltip." + ChangShengJue.MOD_ID + "."
                    + this + ".hold_shift.tooltip", Component.translatable(KungFuConfig.SUNFLOWER_POINT_CAVEMAN_MAX_EXP.get().toString())).withStyle(ChatFormatting.YELLOW);
