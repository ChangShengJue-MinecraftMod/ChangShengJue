...
public class GeShanDaNiuBook extends Item {
    public GeShanDaNiuBook() {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if (!pLevel.isClientSide){
            pPlayer.getCapability(ChangShengJueCapabiliy.KUNGFU).ifPresent(cap -> {
                cap.learnKungFu((ServerPlayer) pPlayer, GeShanDaNiu.KUNG_FU_ID.toString());
                cap.startKungFu((ServerPlayer) pPlayer, GeShanDaNiu.KUNG_FU_ID.toString(), GeShanDaNiu.KUNG_FU_ID.getPath());
            });
        }
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        if (Screen.hasShiftDown()) {
            Component fullDesc = Component.translatable("tooltip." + ChangShengJue.MOD_ID + "."
                    + this + ".hold_shift.tooltip", Component.translatable(KungFuConfig.GE_SHAN_DA_NIU_MAX_EXP.get().toString())).withStyle(ChatFormatting.YELLOW);
