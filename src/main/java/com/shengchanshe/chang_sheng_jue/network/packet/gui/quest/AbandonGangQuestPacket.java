...
// 新增文件：任务提交数据包
public record AbandonGangQuestPacket() {
    public static void encode(AbandonGangQuestPacket packet, FriendlyByteBuf buf) {
    }

    public static AbandonGangQuestPacket decode(FriendlyByteBuf buf) {
        return new AbandonGangQuestPacket();
    }
        ...
        ctx.get().enqueueWork(() -> {
            ...
            if (player.containerMenu instanceof GangQuestsMenu menu) {
                if (menu.getTrader() instanceof AbstractGangLeader abstractGangLeader) {
                    GangQuestHandler.handleAbandonQuest(player, abstractGangLeader);
                }
            }
        });
