...
public class CSJStructureSets {
    // 我们structureset的资源名称
    public static final ResourceKey<StructureSet> SI_HE_YUAN_SET = register("si_he_yuan");
    // 等会使用数据生成要要传递的方法引用
    public static void bootstrap(BootstapContext<StructureSet> pContext) {
        ...
        // 第四个参数是盐
        pContext.register(
                CSJStructureSets.SI_HE_YUAN_SET,
                new StructureSet(holdergetter.getOrThrow(CSJStructures.SI_HE_YUAN), new RandomSpreadStructurePlacement(32, 8, RandomSpreadType.LINEAR, 4241983))
        );
    }

    private static ResourceKey<StructureSet> register(String pName) {
        return ResourceKey.create(Registries.STRUCTURE_SET, new ResourceLocation(ChangShengJue.MOD_ID,pName));
    }
}
