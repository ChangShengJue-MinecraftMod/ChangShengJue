...
public class TurtleBreathWorkHudOverlay {
    ...
    public static final IGuiOverlay HUD_TURTLE_BREATH_WORK = (gui, guiGraphics, partialTick, screenWidth, screenHeight) -> {
        ...
        if (turtleBreathWorkComprehend && turtleBreathWorkOff){
            ...
            if (getTurtleBreathWorkLevel != 0) {//获取技能等级,为零则绘制不可使用的技能贴图
                if (frameTime() <= 0){ //获取技能剩余冷却时间,小于等于0则绘制技能贴图否则绘制冷却中的技能贴图
                    if (playerCanOpened()) {//检查玩家剩余饥饿值,剩余饥饿值不足则绘制冷却中的技能贴图
                        if (getTurtleBreathWorkLevel < 2) {//如果技能等级不为2,绘制普通技能贴图否则绘制大成技能贴图
                            CSJDisplayHud.displayHudPermanent(guiGraphics, TURTLE_BREATH_WORK, x, y);
                        } else {
                            CSJDisplayHud.displayHudPermanent(guiGraphics,TURTLE_BREATH_WORK_1,x,y);
                        }
                    }else {
                        if (getTurtleBreathWorkLevel < 2) {//如果技能等级不为2,绘制普通技能贴图否则绘制大成技能贴图
                            CSJDisplayHud.displayHudPermanent(guiGraphics, TURTLE_BREATH_WORK, x, y);
                    ...
                    float v = ((frameTime() / 20) / 45);
                    int v1 = (int) (16 * v + 1);//计算技能剩余冷却时间
                    if (playerCanOpened()) {//检查玩家剩余饥饿值,剩余饥饿值不足则绘制冷却中的技能贴图并渲染技能剩余冷却时间
                        if (getTurtleBreathWorkLevel < 2) {//如果技能等级不为2,绘制普通技能贴图否则绘制大成技能贴图并渲染技能剩余冷却时间
                            CSJDisplayHud.displayHudPermanent(guiGraphics,TURTLE_BREATH_WORK,x,y);
                            CSJDisplayHud.displayHudPermanent(guiGraphics,COOLING,x,y);
                            guiGraphics.blit(TURTLE_BREATH_WORK, x, y + 20, 0, 0, 0,16, -v1 + 16, 16, 16);
                        } else {
                            CSJDisplayHud.displayHudPermanent(guiGraphics,TURTLE_BREATH_WORK_1,x,y);
                            CSJDisplayHud.displayHudPermanent(guiGraphics,COOLING,x,y);
                            guiGraphics.blit(TURTLE_BREATH_WORK_1, x, y + 20, 0, 0, 0,16, -v1 + 16, 16, 16);
                        }
                        //以文字形式绘制剩余冷却时间
                        guiGraphics.drawString(gui.getFont(),new Formatter().format("%.0f",(v * 45)).toString(),x + 2, y + 25, ChatFormatting.AQUA.getColor());
                    }else {
                        if (getTurtleBreathWorkLevel < 2) {//如果技能等级不为2,绘制普通技能贴图否则绘制大成技能贴图并渲染技能剩余冷却时间
                            CSJDisplayHud.displayHudPermanent(guiGraphics,TURTLE_BREATH_WORK,x,y);
                            CSJDisplayHud.displayHudPermanent(guiGraphics,COOLING,x,y);
                ...
                CSJDisplayHud.displayHudPermanent(guiGraphics,TURTLE_BREATH_WORK_2,x,y);
            }
        }
    };
}
