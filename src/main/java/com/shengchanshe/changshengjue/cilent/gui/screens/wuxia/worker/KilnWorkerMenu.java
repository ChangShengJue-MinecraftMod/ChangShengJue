package com.shengchanshe.changshengjue.cilent.gui.screens.wuxia.worker;
...
public class KilnWorkerMenu extends AbstractContainerMenu {
    ...
    private static final int SELL_SLOT1_X = 135;
    private static final int SELL_SLOT2_X = 161;
    private static final int BUY_SLOT_X = 219;
    private static final int ROW_Y = 80;
    ...
    public KilnWorkerMenu(int id, Inventory inventory) {
        this(id, inventory, new ClientSideMerchant(inventory.player));
    }

    public KilnWorkerMenu(int id, Inventory inventory, Merchant merchant) {
        super(ChangShengJueMenuTypes.KILNWORKER_MENU.get(), id);
        ...
        for(int i = 0; i < 3; ++i) {
            for(int j = 0; j < 9; ++j) {
                this.addSlot(new Slot(inventory, j + i * 9 + 9, 108 + j * 18, 127 + i * 18));
            }
        }

        for(int k = 0; k < 9; ++k) {
            this.addSlot(new Slot(inventory, k, 108 + k * 18, 185));
        }
    ...
    public void setOffers(MerchantOffers offers) {
        ...
        this.tradeContainer.updateSellItem(); // 添加这行以更新容器
    }
