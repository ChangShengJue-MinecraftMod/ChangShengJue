...
public class BlacksmithRenderer extends DynamicGeoEntityRenderer<Blacksmith> {
    ...
    protected final ResourceLocation CAPE_TEXTURE = new ResourceLocation(ChangShengJue.MOD_ID, "textures/entity/blacksmith/blacksmith.png");
        ...
        this.addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            ...
            protected ItemStack getStackForBone(GeoBone bone, Blacksmith animatable) {
                ...
                switch (bone.getName()) {
                    ...
                    case "bipedHandRight" -> var10000 = animatable.isLeftHanded() ? BlacksmithRenderer.this.offhandItem : BlacksmithRenderer.this.mainHandItem;
            ...
            protected ItemDisplayContext getTransformTypeForStack(GeoBone bone, ItemStack stack, Blacksmith animatable) {
            ...
            protected void renderStackForBone(PoseStack poseStack, GeoBone bone, ItemStack stack, Blacksmith animatable, MultiBufferSource bufferSource, float partialTick, int packedLight, int packedOverlay) {
                if (stack == BlacksmithRenderer.this.mainHandItem) {
                    poseStack.mulPose(Axis.XP.rotationDegrees(-90.0F));
                    if (stack.getItem() instanceof ShieldItem) {
                        poseStack.translate(0.0, 0.125, -0.25);
                    }
                } else if (stack == BlacksmithRenderer.this.offhandItem) {
                    poseStack.mulPose(Axis.XP.rotationDegrees(-90.0F));
                    if (stack.getItem() instanceof ShieldItem) {
                        poseStack.translate(0.0, 0.125, 0.25);
    ...
    @Nullable
    protected ResourceLocation getTextureOverrideForBone(GeoBone bone, Blacksmith animatable, float partialTick) {
        return "bipedCape".equals(bone.getName()) ? this.CAPE_TEXTURE : null;
    }

    public void preRender(PoseStack poseStack, Blacksmith animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
