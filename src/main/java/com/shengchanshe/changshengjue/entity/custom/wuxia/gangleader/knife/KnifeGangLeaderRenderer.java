...
public class KnifeGangLeaderRenderer extends DynamicGeoEntityRenderer<KnifeGangLeader> {
...
//    protected final ResourceLocation CAPE_TEXTURE = new ResourceLocation(ChangShengJue.MOD_ID, "textures/entity/gang_leader/knife_gang_leader.png");
    protected ItemStack mainHandItem;
    protected ItemStack offhandItem;

    public KnifeGangLeaderRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new KnifeGangLeaderModel());
        this.addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            @Nullable
            protected ItemStack getStackForBone(GeoBone bone, KnifeGangLeader animatable) {
                if (!animatable.isAggressive()) {
                    return null;
                }
                ItemStack var10000;
                switch (bone.getName()) {
                    case "bipedHandLeft" -> var10000 = animatable.isLeftHanded() ? KnifeGangLeaderRenderer.this.mainHandItem : KnifeGangLeaderRenderer.this.offhandItem;
                    case "bipedHandRight" -> var10000 = animatable.isLeftHanded() ? KnifeGangLeaderRenderer.this.offhandItem : KnifeGangLeaderRenderer.this.mainHandItem;
            ...
            protected ItemDisplayContext getTransformTypeForStack(GeoBone bone, ItemStack stack, KnifeGangLeader animatable) {
            ...
            protected void renderStackForBone(PoseStack poseStack, GeoBone bone, ItemStack stack, KnifeGangLeader animatable, MultiBufferSource bufferSource, float partialTick, int packedLight, int packedOverlay) {
                if (stack == KnifeGangLeaderRenderer.this.mainHandItem) {
                    poseStack.mulPose(Axis.XP.rotationDegrees(-90.0F));
                    if (stack.getItem() instanceof ShieldItem) {
                        poseStack.translate(0.0, 0.125, -0.25);
                    }
                } else if (stack == KnifeGangLeaderRenderer.this.offhandItem) {
                    poseStack.mulPose(Axis.XP.rotationDegrees(-90.0F));
                    if (stack.getItem() instanceof ShieldItem) {
                        poseStack.translate(0.0, 0.125, 0.25);
    ...
    @Nullable
    protected ResourceLocation getTextureOverrideForBone(GeoBone bone, KnifeGangLeader animatable, float partialTick) {
        return "bipedCape".equals(bone.getName()) ? CAPE_TEXTURE.get(animatable.getVariant()) : null;
    }

    public void preRender(PoseStack poseStack, KnifeGangLeader animatable, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
