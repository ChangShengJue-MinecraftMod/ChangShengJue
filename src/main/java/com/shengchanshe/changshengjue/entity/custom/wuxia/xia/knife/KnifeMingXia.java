...
public class KnifeMingXia extends AbstractMingXia implements GeoEntity {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public KnifeMingXia(EntityType<? extends AbstractMingXia> pEntityType, Level pLevel) {
    ...
    protected void populateDefaultEquipmentSlots(RandomSource pRandom, DifficultyInstance pDifficulty) {
        ...
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(ChangShengJueItems.TU_LONG_DAO.get()));
    }
    ...
    private <E extends GeoAnimatable> PlayState attackPredicate(AnimationState<E> animationEvent) {
        if (this.isAttacking() && animationEvent.getController().getAnimationState().equals(AnimationController.State.STOPPED)){
            ...
            switch (i) {
                ...
                case 3 -> animationEvent.setAnimation(RawAnimation.begin().then("attack.right_hand2_knife2_and_sword2", Animation.LoopType.PLAY_ONCE));
                case 4 -> animationEvent.setAnimation(RawAnimation.begin().then("attack.right_hand_sword3_and_spear", Animation.LoopType.PLAY_ONCE));
            }
        }
