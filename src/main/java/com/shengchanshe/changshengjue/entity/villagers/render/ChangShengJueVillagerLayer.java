...
public class ChangShengJueVillagerLayer<T extends LivingEntity & VillagerDataHolder, M extends EntityModel<T> & VillagerHeadModel> extends RenderLayer<T, M> {
    ...
    public ChangShengJueVillagerLayer(RenderLayerParent<T, M> pRenderer, ResourceManager pResourceManager, String pPath) {
        super(pRenderer);
    ...
    public <K> VillagerMetaDataSection.Hat getHatData(Object2ObjectMap<K, VillagerMetaDataSection.Hat> pCache, String pFolder, DefaultedRegistry<K> pVillagerTypeRegistry, K pKey) {
        return pCache.computeIfAbsent(pKey, (p_258159_) -> this.resourceManager.getResource(this.getResourceLocation(pFolder, pVillagerTypeRegistry.getKey(pKey))).flatMap((p_234875_) -> {
            try {
